...
@Component({
    selector: 'update-my-info-cmp',
    moduleId: module.id,
    templateUrl: 'update-my-info.component.html'
})

export class UpdateMyInfoComponent implements OnInit, AfterViewInit {
    ...
    submit() {
        ...
        if (this.isAdmin) {
            data.attach_files = $('#attach-file')[0].files;
        }
        if (this.department) {
        ...
        this.personnelService.updateEmployee(this.id, data).subscribe(
            () => {
                ...
                if (this.isAdmin) {
                    ...
                    text = 'Thay đổi đang được chờ duyệt';
                }
                swal({
                    title: 'Thành công',
                    text: text
